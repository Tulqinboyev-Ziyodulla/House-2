    ...
    <p className='text-sm md:text-base'>Все категории</p>
                ...
                <div  className='flex gap-4 flex-wrap mb-10 items-center justify-center'>
                {
                    box?.map((el) => (
                            ...
                            <div key={el.id} className='h-52  flex flex-col gap-3 p-4 border-x-2 items-center justify-center w-60 relative'>
                                <img className='photo cursor-pointer' src={el.url} alt="" />
                                <p className=' text-xl text-center mt-3 absolute bottom-2'>{el.title}</p>
                            </div>
                    ))
                }
                </div>
